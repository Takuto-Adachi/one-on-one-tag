...
public class player1ver2 : MonoBehaviour
{
    // 移動・視点関連
    ...
    public Transform player1Camera; // 子オブジェクトの参照
    ...
    void Start()
    {
        player1Camera = transform.Find("Player1Camera");
    }
    ...
    void Update()
    {
        ...
        //視点移動関連
        if(Input.GetKey(KeyCode.Z)) //playerの左右回転
        {
            rot.y -= rotspeed; //playerが反時計回りに回転
        }

        if(Input.GetKey(KeyCode.C)) //playerの横回転
        {
            rot.y += rotspeed; //playerが時計回りに回転
        }

        if(Input.GetKey(KeyCode.D)) //playerのカメラを上下回転
        {
            //playerのカメラが下に回転
            rot.x -= rotspeed;
        ...
        if(Input.GetKey(KeyCode.E)) //playerのカメラを上下回転
        {
            //playerのカメラが上に回転
            rot.x += rotspeed;
        ...
        player1Camera.localEulerAngles = new Vector3(rot.x, 0, 0);  //首の向き（cameraの向き）
        ...
        if(isTouch == true) //playerの捕まる判定
        {
            ...
            loseplayer1.SetActive(true); // 負け演出
            winplayer2.SetActive(true); // 勝ち演出
        ...
        // 加速関連
        if (Input.GetKey(KeyCode.A) && limit < maxlimit && isOutstock == false) //playerの加速
